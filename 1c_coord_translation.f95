SUBROUTINE coord_translation(guest_atom, host_atom, nat, n_samples, a, b, c)
      ...
      oxygen: if (TRIM(wannier_center_info(iatom, i)%wannier_center_name) == TRIM(host_atom)) then
        inner2: DO jatom = 1, nat
          hydrogen: if (TRIM(wannier_center_info(jatom, i)%wannier_center_name) == TRIM(guest_atom)) then
              ...
              A1 =  wannier_center_info(iatom, i)%coord(1) < (-a/2)+R_OD_c .AND. wannier_center_info(iatom, i)%coord(1) > -a/2    &
              .AND. wannier_center_info(jatom, i)%coord(1) < a/2       .AND. wannier_center_info(jatom, i)%coord(1) > a/2-R_OD_c  
              B1 =  wannier_center_info(iatom, i)%coord(2) < (-b/2)+R_OD_c .AND. wannier_center_info(iatom, i)%coord(2) > -b/2    &
              .AND. wannier_center_info(jatom, i)%coord(2) < b/2       .AND. wannier_center_info(jatom, i)%coord(2) > b/2-R_OD_c
              C1 =  wannier_center_info(iatom, i)%coord(3) < (-c/2)+R_OD_c .AND. wannier_center_info(iatom, i)%coord(3) > -c/2    &
              .AND. wannier_center_info(jatom, i)%coord(3) < c/2       .AND. wannier_center_info(jatom, i)%coord(3) > c/2-R_OD_c 
              A2 =  wannier_center_info(jatom, i)%coord(1) < (-a/2)+R_OD_c .AND. wannier_center_info(jatom, i)%coord(1) > -a/2    &
              .AND. wannier_center_info(iatom, i)%coord(1) < a/2       .AND. wannier_center_info(iatom, i)%coord(1) > a/2-R_OD_c  
              B2 =  wannier_center_info(jatom, i)%coord(2) < (-b/2)+R_OD_c .AND. wannier_center_info(jatom, i)%coord(2) > -b/2    &
              .AND. wannier_center_info(iatom, i)%coord(2) < b/2       .AND. wannier_center_info(iatom, i)%coord(2) > b/2-R_OD_c   
              C2 =  wannier_center_info(jatom, i)%coord(3) < (-c/2)+R_OD_c .AND. wannier_center_info(jatom, i)%coord(3) > -c/2    &
              .AND. wannier_center_info(iatom, i)%coord(3) < c/2       .AND. wannier_center_info(iatom, i)%coord(3) > c/2-R_OD_c 
            ...
            elseif (                            & 
              ...
              !For testing
              !===========
              !WRITE(*,*) "image x: ", wannier_center_info(jatom, i)%image_coord(1) - wannier_center_info(jatom, i)%coord(1)  
              !WRITE(*,*) "image y: ", wannier_center_info(jatom, i)%image_coord(2) - wannier_center_info(jatom, i)%coord(2)
              !WRITE(*,*) "image z: ", wannier_center_info(jatom, i)%image_coord(3) - wannier_center_info(jatom, i)%coord(3)
            ...
            ! elseif(
            !        x(host) < (-a/2)+R_OD_c .and. x(host) > (-a/2)    .AND.      x(guest) < a/2 .and. x(host) > a/2-R_OD_c    .... A1
            ! .AND.  y(host) < (-b/2)+R_OD_c .and. y(host) > (-b/2)    .AND.      y(guest) < b/2 .and. y(host) > b/2-R_OD_c    .... B1
            ! .AND.  z(host) < (-c/2)+R_OD_c .and. z(host) > (-c/2)    .AND.      z(guest) < c/2 .and. z(host) > c/2-R_OD_c    .... C1 
            !    ) then
            !=========================================================================================================================
            elseif (                             &
              A1 .AND. B1 .AND. C1               &
              ...
              !=====================================================================================================================
              ! The real meanin of the logical expr. (A1 .AND. B1 .AND. C1) is as follows.
              !      wannier_center_info(iatom, i)%coord(1) < (-a/2)+R_OD_c .AND. wannier_center_info(iatom, i)%coord(1) > -a/2    &
              !.AND. wannier_center_info(jatom, i)%coord(1) < a/2       .AND. wannier_center_info(jatom, i)%coord(1) > a/2-R_OD_c  & 
              !.AND. wannier_center_info(iatom, i)%coord(2) < (-b/2)+R_OD_c .AND. wannier_center_info(iatom, i)%coord(2) > -b/2    &
              !.AND. wannier_center_info(jatom, i)%coord(2) < b/2       .AND. wannier_center_info(jatom, i)%coord(2) > b/2-R_OD_c  & 
              !.AND. wannier_center_info(iatom, i)%coord(3) < (-c/2)+R_OD_c .AND. wannier_center_info(iatom, i)%coord(3) > -c/2    &
              !.AND. wannier_center_info(jatom, i)%coord(3) < c/2       .AND. wannier_center_info(jatom, i)%coord(3) > c/2-R_OD_c 
              !=====================================================================================================================
  ...
  OPEN (UNIT=u,FILE='TEST_OUTPUT.dat',STATUS='REPLACE',ACTION='WRITE',IOSTAT=ierror)
    WRITE(u, *) 'Coord. of Guest_atom ', ' image_coord ', ' coord ', ' difference '
  do i = 1, n_samples 
    do jatom = 1, nat 
      if (TRIM(wannier_center_info(jatom, i)%wannier_center_name) == "H") then
        WRITE(u,*) 'coord x for D:', &
                  wannier_center_info(jatom, i)%image_coord(1), wannier_center_info(jatom, i)%coord(1), &
                  wannier_center_info(jatom, i)%image_coord(1) - wannier_center_info(jatom, i)%coord(1)
        WRITE(u,*) 'coord y for D:', &
                  wannier_center_info(jatom, i)%image_coord(2), wannier_center_info(jatom, i)%coord(2), &
                  wannier_center_info(jatom, i)%image_coord(2) - wannier_center_info(jatom, i)%coord(2)
        WRITE(u,*) 'coord z for D:', &
                  wannier_center_info(jatom, i)%image_coord(3), wannier_center_info(jatom, i)%coord(3), &
                  wannier_center_info(jatom, i)%image_coord(3) - wannier_center_info(jatom, i)%coord(3)
      else if (TRIM(wannier_center_info(jatom, i)%wannier_center_name) == "X") then
        WRITE(u,*) 'coord x for X:', &
                  wannier_center_info(jatom, i)%image_coord(1), wannier_center_info(jatom, i)%coord(1), &
                  wannier_center_info(jatom, i)%image_coord(1) - wannier_center_info(jatom, i)%coord(1)
        WRITE(u,*) 'coord y for X:', &
                  wannier_center_info(jatom, i)%image_coord(2), wannier_center_info(jatom, i)%coord(2), &
                  wannier_center_info(jatom, i)%image_coord(2) - wannier_center_info(jatom, i)%coord(2)
        WRITE(u,*) 'coord z for X:', &
                  wannier_center_info(jatom, i)%image_coord(3), wannier_center_info(jatom, i)%coord(3), &
                  wannier_center_info(jatom, i)%image_coord(3) - wannier_center_info(jatom, i)%coord(3)
      end if
    end do
